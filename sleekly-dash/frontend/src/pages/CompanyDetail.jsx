    ...
    <div className="space-y-4">
      <div className="bg-white p-4 rounded-xl shadow flex justify-between">
        <div>
          ...
          {company.discovery_account_id && (
            <div className="mt-2 text-xs text-cyan-700 font-medium">
              Discovery · {company.discovery_account_id}
            </div>
          )}
        </div>
      ...
      {(company.status === 'closed_won' || company.status === 'closed_lost') && (
        <div className="bg-white p-4 rounded-xl shadow grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          <div>
            <div className="text-gray-500">Closed</div>
            <div className="font-medium">{company.closed_at || '—'}</div>
          </div>
          <div>
            <div className="text-gray-500">Project value (UGX)</div>
            <div className="font-medium">
              {company.project_value_ugx != null
                ? Number(company.project_value_ugx).toLocaleString()
                : '—'}
            </div>
          </div>
          <div>
            <div className="text-gray-500">
              {company.status === 'closed_lost' ? 'Loss reason' : 'Services sold'}
            </div>
            <div className="font-medium">
              {company.status === 'closed_lost'
                ? company.loss_reason || '—'
                : Array.isArray(company.services_sold)
                  ? company.services_sold.join(', ')
                  : company.services_sold || '—'}
            </div>
          </div>
        </div>
      )}
