...
class CompanyController {
    ...
    public function update($id) {
        ...
        $fields = [
            ...
            'discovery_account_id', 'closed_at', 'project_value_ugx', 'services_sold', 'loss_reason',
        ];
        ...
        $set = [];
        $params = [':id' => $id];
        $incomingStatus = array_key_exists('status', $data) ? $data['status'] : null;
        ...
        foreach ($fields as $f) {
            ...
            } elseif ($f === 'project_value_ugx') {
                $set[] = 'project_value_ugx = :project_value_ugx';
                $params[':project_value_ugx'] = $data['project_value_ugx'] === null || $data['project_value_ugx'] === ''
                    ? null
                    : max(0, (int) $data['project_value_ugx']);
            } elseif ($f === 'services_sold') {
                $set[] = 'services_sold = :services_sold';
                if ($data['services_sold'] === null || $data['services_sold'] === '') {
                    $params[':services_sold'] = null;
                } elseif (is_string($data['services_sold'])) {
                    $params[':services_sold'] = $data['services_sold'];
                } else {
                    $params[':services_sold'] = json_encode($data['services_sold']);
                }
            } elseif ($f === 'discovery_account_id' || $f === 'loss_reason' || $f === 'closed_at') {
                $val = $data[$f];
                if (is_string($val)) {
                    $val = trim($val);
                    $val = $val === '' ? null : $val;
        ...
        // Auto-stamp closed_at when moving into a closed status without an explicit closed_at.
        if (
            is_string($incomingStatus)
            && in_array($incomingStatus, ['closed_won', 'closed_lost'], true)
            && !array_key_exists('closed_at', $data)
        ) {
