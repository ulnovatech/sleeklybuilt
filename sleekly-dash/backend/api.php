...
try {
    ...
    // Auth routes (public)
    if ($path === '/api/auth/capabilities' && $method === 'GET') {
        $authController->capabilities();
        exit;
    }
    ...
    if (preg_match('#^/api/auth/users/(\d+)$#', $path, $m) && $method === 'DELETE') {
        $authController->deactivateUser((int) $m[1]);
        exit;
    }
    ...
    $integrationsController = new IntegrationsController($pdo, $requestsController);
    ...
    if (strpos($path, '/api/integrations') === 0) {
        if ($method === 'POST' && $path === '/api/integrations/prospects') {
            echo json_encode($integrationsController->upsertProspect(json_body()));
        } elseif ($method === 'GET' && $path === '/api/integrations/outcomes') {
            echo json_encode($integrationsController->outcomes($_GET));
        } elseif ($method === 'GET' && $path === '/api/integrations/catalog') {
            echo json_encode($integrationsController->catalog());
        } elseif ($method === 'GET' && $path === '/api/integrations/inbound') {
            echo json_encode($integrationsController->inbound($_GET));
        } else {
            http_response_code(404);
