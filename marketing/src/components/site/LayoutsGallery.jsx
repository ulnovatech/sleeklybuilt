...
function LayoutCard({ layout, ctaLabel }) {
  const fitLabel = layout.layoutFit ? getLayoutFit(layout.layoutFit)?.label : null
  const eyebrow = fitLabel || layout.category
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-dos-xl border border-subtle bg-surface-raised shadow-sm transition duration-fast ease-dos hover:border-action-primary/30 hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        {eyebrow ? <span className="eyebrow text-emerald">{eyebrow}</span> : null}
...
export default function LayoutsGallery({
  ...
  const filterEmptyTitle = (() => {
    if (filters.layoutFitId) {
      const fit = getLayoutFit(filters.layoutFitId)
      return `No layouts for ${fit?.label ?? 'this fit'} yet`
    }
    if (filters.businessTypeId) {
      const type = getBusinessType(filters.businessTypeId)
      return `No layouts for ${type?.label ?? 'this business type'} yet`
    }
    return 'No layouts match that search'
  })()
  ...
  return (
    <Section id={id} className="section-light scroll-mt-24">
      ...
      <SectionBody>
        {stale ? (
          ...
          >
            <p className="text-body text-ink">
              Showing the last loaded layouts — a refresh just failed. You can keep browsing or try
              again.
            </p>
            <button
              type="button"
        ...
        <div
          className={cn(state === 'loading' && layouts.length > 0 && 'opacity-60 transition-opacity duration-fast')}
        >
          {firstLoad ? (
            <>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-emerald-deep">
                    {forbidden
                      ? 'This gallery is not available right now'
                      : 'We could not load these layouts'}
                  </p>
