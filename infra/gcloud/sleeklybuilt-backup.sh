#!/usr/bin/env bash
# Daily MySQL + Postgres dumps on the GCE VM. Installed to /usr/local/bin/sleeklybuilt-backup.sh
set -euo pipefail
...
docker compose -f infra/docker-compose.full.yml exec -T mysql \
  mysqldump -u root -p"${MYSQL_ROOT_PASSWORD}" --single-transaction sleeklybuilt \
