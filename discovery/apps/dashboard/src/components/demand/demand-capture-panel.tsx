...
export function DemandCapturePanel({ onCreated }: { onCreated?: () => void }) {
      ...
      setMsg(
        result.created
          ? result.matchedBusinessId
            ...
            : 'Demand signal created — triage it in the Inbox tab.'
  ...
  return (
    <div className="max-w-3xl space-y-6">
      {msg && (
        <p className="text-sm text-green-800 bg-green-50 border border-green-200 rounded p-3">{msg}</p>
      )}
      {error && (
        <p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded p-3">{error}</p>
      )}

      <form onSubmit={submitPaste} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3">
